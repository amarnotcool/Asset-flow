...
const Reports = () => {
  ...
  const { assets, departments, bookings, maintenanceTickets, allocationHistory } = useAppStore();
    ...
    <div ref={reportRef} className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="card flex flex-col">
          <h3 className="font-bold text-lg flex items-center gap-2 m-0 text-text-primary mb-6">
          ...
          <div className="flex flex-col gap-3 flex-1 overflow-y-auto custom-scrollbar pr-1 max-h-64">
            {departments.map(dept => (
              <div key={dept.id} className="flex justify-between items-center p-3 rounded-lg bg-bg-primary border border-border-color">
                ...
                <span className={`badge uppercase tracking-wider text-[10px] ${dept.status === 'Active' ? 'badge-success' : 'badge-neutral'}`}>{dept.status}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <ul className="list-none text-sm text-text-primary space-y-3 m-0 p-0">
            {idleAssets.length > 0 ? idleAssets.map(a => (
              <li key={a.id} className="flex justify-between border-b border-border-color pb-3 last:border-0 last:pb-0">
                <div>
                  <span className="font-bold text-accent-primary block">{a.tag}</span>
                  <span className="text-xs text-text-secondary">{a.name}</span>
                </div>
                <div className="text-right">
                  <span className="badge badge-success text-[10px]">Available</span>
                  <span className="block text-[10px] text-text-secondary mt-1">No recent allocations</span>
                </div>
              </li>
            )) : (
              <li className="text-xs text-text-secondary italic">No idle assets found.</li>
            )}
          </ul>
        </div>
        ...
        <div className="card bg-alert-warning-bg/30 border-alert-warning/50">
          ...
          <ul className="list-none text-sm text-text-primary space-y-3 m-0 p-0">
            {maintenanceFreq.length > 0 ? maintenanceFreq.map(([tag, count], idx) => {
              const asset = assets.find(a => a.tag === tag);
              return (
                <li key={idx} className="flex justify-between border-b border-alert-warning/20 pb-3 last:border-0 last:pb-0">
                  <div>
                    <span className="font-bold text-text-primary block">{tag}</span>
                    <span className="text-xs text-text-secondary">{asset?.name || 'Unknown'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="badge badge-warning text-[10px] border-alert-warning/30 shadow-sm">{count} Tickets</span>
                  </div>
                </li>
              );
            }) : (
              <li className="text-xs text-text-secondary italic">No maintenance history available.</li>
            )}
          </ul>
        </div>
      </div>
