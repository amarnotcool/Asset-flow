...
const AssetDirectory = () => {
  ...
  const { assets, categories, registerAsset, allocationHistory, maintenanceTickets } = useAppStore();
  ...
  const resetForm = () => {
    ...
    setSerialNumber(''); setAcquisitionDate(''); setIsShared(false);
    setAssetCondition('New'); setAssetCategory('Electronics');
  };
              ...
              {filteredAssets.length > 0 ? (
                filteredAssets.map((asset) => (
                  <tr key={asset.id} onClick={() => setSelectedAsset(asset)} className="hover:bg-bg-primary/50 transition-colors cursor-pointer group">
                    ...
                    <td className="td font-semibold text-text-primary">{asset.name} {asset.isShared && <span className="badge badge-info ml-2 text-[10px] py-0">Shared</span>}</td>
                    <td className="td text-text-secondary">{asset.category}</td>
                    <td className="td"><span className={`badge ${getStatusBadge(asset.status)}`}>{asset.status}</span></td>
                    <td className="td text-text-primary">{asset.holder || '—'}</td>
          ...
          <div className="fixed top-0 right-0 h-full w-[400px] max-w-[90vw] bg-bg-secondary border-l border-border-color shadow-2xl z-50 flex flex-col transform transition-transform duration-300">
            ...
            <div className="p-5 border-b border-border-color flex justify-between items-center bg-bg-primary">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-text-primary m-0">{selectedAsset.tag}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6 custom-scrollbar">
              ...
              <div>
                ...
                <div className="bg-bg-primary border border-border-color rounded-xl p-4 grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-text-primary">{selectedAsset.holder}</span>
                  </div>
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-text-primary">{selectedAsset.category}</span>
                  </div>
                  <div>
                    <span className="block text-text-secondary mb-1">Shared Resource</span>
                    <span className="font-semibold text-text-primary">{selectedAsset.isShared ? 'Yes' : 'No'}</span>
                  </div>
                  <div>
                    <span className="block text-text-secondary mb-1">Serial Number</span>
                    <span className="font-semibold text-text-primary">{selectedAsset.serialNumber || '—'}</span>
                  </div>
                  <div>
                    <span className="block text-text-secondary mb-1">Acquisition</span>
                    <span className="font-semibold text-text-primary">{selectedAsset.acquisitionDate || '—'}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-3 flex justify-between items-center">
                  ...
                  <span className="badge badge-neutral text-[10px]">{getAssetHistory(selectedAsset.tag).length} records</span>
                </h4>
                ...
                <div className="relative pl-6">
                  ...
                  {getAssetHistory(selectedAsset.tag).length > 0 ? (
                    <div className="flex flex-col gap-5">
                      {getAssetHistory(selectedAsset.tag).map((record, idx) => (
            ...
            <form onSubmit={handleRegisterSubmit} className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" className="input" placeholder="e.g. SN-12345ABC" value={serialNumber} onChange={e => setSerialNumber(e.target.value)} />
                </div>
                <div>
